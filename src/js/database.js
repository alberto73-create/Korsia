...
export async function loadCameraPackage(){let pkg=await get(KEY);if(pkg)return pkg;const r=await fetch('/data/cameras-it-demo.json');pkg=await r.json();await put(KEY,pkg);return pkg}
export async function updateDemoPackage(){const r=await fetch('/data/cameras-it-demo.json',{cache:'no-store'});if(!r.ok)throw new Error('Download non riuscito');const pkg=await r.json();pkg.downloadedAt=new Date().toISOString();await put(KEY,pkg);return pkg}
